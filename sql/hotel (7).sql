...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-03-2022 a las 20:34:58
...
(4, 'Abagail', 'Nienow', 620891333, '22157257h', 'arunolfsdottir@yahoo.com', '354 Lehner Turnpike\nWest Margaretttown, VA 71730-3571', 'Q9HZ\"7*Uz\'9^iS`xP', 2, '', '2022-03-10 18:32:34', '2022-03-10 18:32:34'),
...
(99, 'Elza', 'Russel', 693139193, '91263481l', 'enoch.nolan@torp.com', '166 Koepp Station Suite 265\nNorth Lenoreburgh, UT 21415-5107', '6A:%qM|Q\'d', 2, '', '2022-03-10 18:32:35', '2022-03-10 18:32:35'),
...
(103, 'Ernest', 'Segui Gonzalez', 654987312, '65421987Q', 'dawbegonzalezsegui@iesjoanramis.org', 'C/ Mahon, 12', '123456', 2, 'albaricoque700.png', '2022-03-29 15:11:14', '2022-03-29 15:11:14');
...
--
-- Volcado de datos para la tabla `64_reserves`
--
...
--
-- Estructura Stand-in para la vista `64_reserves_view`
...
--
-- Estructura para la vista `64_reserves_view`
--
DROP TABLE IF EXISTS `64_reserves_view`;

DROP VIEW IF EXISTS `64_reserves_view`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `64_reserves_view`  AS SELECT `reserves`.`reserva_id` AS `ID`, `reserves`.`entrada` AS `Entrada`, `reserves`.`salida` AS `Salida`, concat(`clients`.`nombre`,' ',`clients`.`apellido`) AS `Nombre Completo Usuario`, `clients`.`tlf` AS `Telefono`, `clients`.`dni` AS `DNI`, `clients`.`email` AS `Email`, `tipushabitacio`.`nombre` AS `Tipus Habitacio`, `tipushabitacio`.`id` AS `ID Tipus Habitacio`, `habitacio`.`numero` AS `Número Habitación`, `estat`.`nom` AS `Estado Reserva`, `reserves`.`precio` AS `Precio reserva`, `pensio`.`nom` AS `Tipo Pensión` FROM (((((`64_reserves` `reserves` join `64_clients` `clients` on(`reserves`.`cliente_id` = `clients`.`cliente_id`)) join `64_habitacio` `habitacio` on(`reserves`.`habitacion_id` = `habitacio`.`id`)) join `64_tipushabitacio` `tipushabitacio` on(`habitacio`.`tipo_habitacion` = `tipushabitacio`.`id`)) join `64_estat_reserva` `estat` on(`reserves`.`estado_id` = `estat`.`id`)) join `64_pensio` `pensio` on(`reserves`.`pension_id` = `pensio`.`id`)) ;
...
--
ALTER TABLE `64_clients`
  MODIFY `cliente_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=106;
